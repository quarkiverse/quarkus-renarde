...
=== Out of the box support for form logins

If you don't want to customise your login screen, and are using user/password security, you can opt to implement
`RenardeUserWithPassword`, and Renarde will take care of the login redirect and use a JWT cookie for your login:
...
You will also automatically get a login page with a form when attempting to log in, using your user table.

NOTE: This automatic form is only enabled if you have no `@LoginPage` endpoint, and you have a `RenardeUserWithPassword`
implementation.

You can now use the `@Authenticated` annotation on your endpoints.

=== Manual user registration and login

If you want to implement a full-fledged user login and registration experience, with passwords, your entity should extend
`RenardeUser` and can look like this:
...
----
<1> `@LoginPage` allows you to specify where the user should be redirected when they need to log in.
