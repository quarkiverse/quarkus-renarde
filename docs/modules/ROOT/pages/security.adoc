= Renarde 🦊 Web Framework Security guide

include::./includes/attributes.adoc[]

Renarde 🦊 can help you deal with security and user registration, either manually, using OIDC, or
a mix of both.
...
Start with importing the `renarde-oidc` module:
...
You can make sure that you produce your user type in the request scope with this:
...
A `ControllerWithUser` class might be handy to access the current user for all controllers
that need a user:
