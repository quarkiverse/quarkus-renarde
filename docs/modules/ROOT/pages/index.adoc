...
If you want to use htmx with Renarde, we added some an extra layer to make things more convenient:
[source,java]
----
public class Application extends HxController {
    public TemplateInstance foo() {
        if (isHxRequest()) { // <1>
            return  concatTemplates( // <2>
                    Templates.foo$a(),
                    Templates.foo$b()
            );
        }
    ...
    public TemplateInstance bar() {
        onlyHxRequest(); // <3>
        this.hx(HxResponseHeader.TRIGGER, "refresh"); // <4>
        return Templates.bar();
    }
}
----
<1> Check if this is a htmx request by looking for the `HX-Request` header or using flash data for redirects
<2> https://htmx.org/attributes/hx-swap-oob/[Out of band swap] with different templates or https://quarkus.io/guides/qute-reference#fragments[fragments]
<3> Only Hx requests are allowed, else it will fail with a BAD_REQUEST error
<4> Flag the response with an https://htmx.org/reference/#response_headers[htmx response header]
