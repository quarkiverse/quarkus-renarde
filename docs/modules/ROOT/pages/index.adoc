...
If you want to use this extension, you need to add the `io.quarkiverse.renarde:quarkus-renarde` extension first.
...
Vixen is a server-side Web Framework based on Quarkus, xref:qute-reference.adoc[Qute], 
...
<dependency>
  ...
  <artifactId>quarkus-vixen</artifactId>
</dependency>
----

== First: an example

Let's see how you can quickly build a Web Application with Vixen. Let's start with a _Controller_:
...
Vixen comes with a few extra tags to make your life easier:
...
|`{#form uri method='POST' klass='css'}...{/form}`
|Generates an HTML form for the given `URI`, `method` (defaults to `POST`) and optional CSS classes. Includes a CRSF token.
...
Creating forms in Vixen is easy: let's see an example of how to do it in Qute:

[source,html]
----
{#form uri:Register.complete(newUser.confirmationCode)}
...
As for the `form` Vixen tag, it is also fairly simple, and only includes an authenticity token for CRSF protection.
...
<p>
 If you want to register, <a href="{uriabs:Register.confirm(user.confirmationCode)}">complete your registration</a>.
</p>
{/include}
----
...
{uriabs:Register.confirm(user.confirmationCode)}
{/include}
----
