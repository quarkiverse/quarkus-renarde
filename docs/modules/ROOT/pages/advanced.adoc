...
NOTE: If you need to pass your localised message to another qute tag, or you need to invoke `String` methods on the
rendered message, you can force the rendering of your message to a `String` by passing it parameters, such as in the
previous example with `m:views_Application_index_greet(name)` but if your message does not require parameters you
can force the rendering with the `$render` method:

[source,txt]
----
This will be rendered in your document:
{m:hello}
But if you need to turn it into a String in your template expression, to pass it to a Qute tag, or to invoke String
methods such as toUpperCase, you can use $render to render your message into a String:
{m:hello.$render.toUpperCase}
----
...
The best way to generate dynamic images is using Qute templating to render HTML with CSS. Just like for PDF, this is
simply the easiest way to develop your graphics (using the HTML rendering and browser debug tools) and maintain your
graphics.

To generate an image, it is exactly the same setup as generating a PDF, except you mark your endpoint as producing
a PNG:
...
public class Application extends Controller {
    ...
    // This will return a PNG file to the caller
    @Produces(Pdf.IMAGE_PNG)
    public TemplateInstance png() {
        return Templates.page();
    }
...
=== Caveats of HTML rendering and limitations

https://github.com/danfickle/openhtmltopdf[OpenHTMLToPDF], the engine behind the magic, is based on the
https://github.com/flyingsaucerproject/flyingsaucer[Flying Saucer engine]. It is *not* a browser and never pretends
to be one. That’s why some designs render beautifully in your browser but fall apart when converted to PDF or PNG.
...
- Modern filters (`blur`, `drop-shadow`, `backdrop-filter`)
...
- `.otf` files will be ignored, and fallback fonts are limited.
...
- Complex `linear-gradient()` or `radial-gradient()` may not appear
- Shadows with transparency may get flattened
